    ...
    def capture(self, index):
        ...
        if self.latest_rgb is not None and self.latest_depth is not None:
            ...
            depth_vis = cv2.normalize(
                np.nan_to_num(self.latest_depth),
                None,
                0,
                255,
                cv2.NORM_MINMAX,
            ...
            cv2.imwrite(
                os.path.join(self.save_dir, f"depth_vis_{index:03d}.png"), depth_vis
            )
...
def main():
    ...
    parser.add_argument("--z", type=float, default=0.2, help="初始相机Z位置")
    parser.add_argument("--steps", type=int, default=12, help="采集步数")
    parser.add_argument(
        "--target_name", type=str, default="realsense_d405", help="相机模型名称"
    )
    parser.add_argument(
        "--strawberry_xyz",
        type=float,
    ...
    print(
        f"初始设定: ({args.x}, {args.y}, {args.z}) -> 半径: {radius:.2f}, 相对高度: {height:.2f}"
    )
    ...
    try:
        for i in range(steps):
            ...
            print(
                f"Moving to step {i+1}/{steps}: pos=({cam_x:.2f}, {cam_y:.2f}, {cam_z:.2f}), yaw={math.degrees(yaw):.1f}°"
            )
